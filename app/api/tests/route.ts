...
export async function GET() {
  try {
    // Read allowed for all authenticated staff (used in request forms).
    ...
    const allTests = await db
      ...
      .where(and(eq(labTests.organisationId, orgId), isNull(labTests.deletedAt)))
...
export async function POST(request: NextRequest) {
  try {
    ...
    void logAudit({
      ...
      entityType: "lab_test",
      entityId: newTest.id,
      details: { name: newTest.name, price: newTest.price, departmentId: newTest.departmentId },
    });
