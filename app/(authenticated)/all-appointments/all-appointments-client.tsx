...
import {CalendarIcon, CheckCircleIcon, XCircleIcon, ExclamationCircleIcon} from "@heroicons/react/24/outline";
...
export default function AllAppointmentsClient({session} : AllAppointmentsClientProps) {
    ...
    const columns = useMemo(() => [
        ...
        columnHelper.accessor("patientGender", {
            header: "Gender",
            cell: (info) => <span className="capitalize">
                {
                info.getValue()
            }</span>
        }),
                    ...
                    return allStatuses.map((status) => {
                        ...
                        return (
                            <div key={status}
                                ...
                                <div className={
                                    `absolute -right-6 -bottom-6 w-24 h-24 rounded-full opacity-10 bg-linear-to-br ${
                                        config.gradient
            ...
            isLoading ? (
                ...
                <> {/* Table with Pagination */}
                    <Table data={appointments}
                        columns={columns}/> {/* Statistics Footer Removed */} </>
            )
        }

            {/* Detail Modal */}
            <StatusAppointmentsModal isOpen={
                    !!selectedStatus
                }
                onClose={
