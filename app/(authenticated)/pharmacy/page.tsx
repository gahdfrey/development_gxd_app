...
import { TruckIcon, XCircleIcon, CheckCircleIcon, MagnifyingGlassIcon } from "@heroicons/react/24/outline";
...
function CancelModal({ prescription, onClose, onConfirm }: {
  ...
  const handleConfirm = async () => {
    ...
    catch { setError("Failed to cancel. Try again."); }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="px-6 py-5 border-b border-gray-100">
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {prescription.productName} — {prescription.patientFirstname} {prescription.patientLastname}
          </p>
        </div>
        <div className="px-6 py-5 space-y-3">
          ...
          <div>
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)}
              placeholder="Explain why this prescription is being cancelled…" rows={3}
...
export default function PharmacyPage() {
  const { data, isLoading, mutate } = useSWR<PrescriptionRow[]>("/api/prescriptions", fetcher);
  const [search, setSearch] = useState("");
  const [actionLoading, setActionLoading] = useState<number | null>(null);
  const [cancelTarget, setCancelTarget] = useState<PrescriptionRow | null>(null);
  ...
  const handleDispatch = async (id: number) => {
    ...
    try {
      ...
      if (res.ok) mutate();
      else { const d = await res.json(); alert(d.error ?? "Failed to dispatch"); }
  ...
  const handleCancel = async (id: number, reason: string) => {
    ...
    if (res.ok) { mutate(); setCancelTarget(null); }
    else { const d = await res.json(); throw new Error(d.error ?? "Failed to cancel"); }
  };
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Manage drug prescriptions — dispatch or cancel</p>
      </div>

      {/* Search */}
      <div className="relative max-w-sm">
        <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 pointer-events-none" />
        <input type="text" placeholder="Search patient or drug…"
                    ...
                    return (
                      <tr key={row.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-5 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${PAYMENT_BADGE[row.paymentStatus] ?? "bg-gray-100 text-gray-600 border-gray-300"}`}>
                            {row.paymentStatus === "paid" ? "Paid" : "Not Paid"}
                          </span>
                        </td>
                        <td className="px-5 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${STATUS_BADGE[row.status] ?? "bg-gray-100 text-gray-600 border-gray-300"}`}>
                            ...
                            {isPending && (
                              <>
                                <button
                                  ...
                                  title={!isPaid ? "Payment required before dispatch" : undefined}
              ...
              return (
                <div key={row.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-sm font-bold text-gray-900">{row.patientFirstname} {row.patientLastname}</p>
                      <p className="text-xs text-gray-500">{row.patientDob ? formatAge(row.patientDob) : "—"} · Dr. {row.requestedByFirstname} {row.requestedByLastname}</p>
                    </div>
                    <span className={`inline-flex shrink-0 items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${STATUS_BADGE[row.status] ?? "bg-gray-100 text-gray-600 border-gray-300"}`}>
                      {row.status.charAt(0).toUpperCase() + row.status.slice(1)}
                    </span>
                  </div>
                  <div className="px-3 py-2.5 rounded-lg bg-gray-50 border border-gray-100">
                    ...
                    {row.productPrice != null && <p className="text-xs text-gray-500 mt-0.5">₦{row.productPrice.toLocaleString()}</p>}
                  </div>
                  <div className="flex items-center gap-2 flex-wrap pt-1 border-t border-gray-100">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold border ${PAYMENT_BADGE[row.paymentStatus] ?? "bg-gray-100 text-gray-600 border-gray-300"}`}>
                      {row.paymentStatus === "paid" ? "Paid" : "Not Paid"}
                    </span>
                    {isPending && (
                      <>
                        <button onClick={() => handleDispatch(row.id)} disabled={busy || !isPaid}
                          ...
                          <TruckIcon className="h-3.5 w-3.5" />{busy ? "…" : "Dispatch"}
                        </button>
                        <button onClick={() => setCancelTarget(row)} disabled={busy}
                    ...
                    {isDispatched && (
                      <span className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg opacity-60 ml-auto">
                        <CheckCircleIcon className="h-3.5 w-3.5" />Dispatched
                      </span>
                    )}
                  </div>
                </div>
