...
export default function EditUserForm({ user, onSubmit, onCancel }: EditUserFormProps) {
  ...
  const { data: roles, isLoading: rolesLoading } = useSWR<{ id: number; name: string }[]>(
    "/api/roles",
    fetcher,
  );
  ...
  const onFormSubmit = async (data: UserFormData) => {
    ...
    try {
      await onSubmit({ ...data, roleId: data.roleId ? data.roleId : undefined });
    } catch (err) {
      console.error(err);
    }
  };

  // Wait for roles before mounting the form so defaultValues finds a matching <option>
